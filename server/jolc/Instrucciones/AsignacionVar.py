        ...
        if myVarOld == None:
            ...
            if isinstance(self.expresion, Primitivo):
                ...
                if isinstance(resultadoExpresion, list):
                    ...
                    c3dObj.addContadorT()
                    myNewVariable = VariableC3D(self.identificador, "P + " + str(c3dObj.getNumVariables() - 1), TipoVar.VALOR, self.expresion.type, TipoVariable.VARIABLE, self.line, self.column)
                else:
                    C3D += resultadoExpresion
                    C3D += c3dObj.endString()
                    temporalT0 = c3dObj.getLastContadorT()
                    myNewVariable = VariableC3D(self.identificador, "P + " + str(c3dObj.getNumVariables() - 1), TipoVar.APUNTADOR, self.expresion.type, TipoVariable.VARIABLE, self.line, self.column)
                ...
                resultadoExpresion = self.expresion.getC3D(c3dObj)
                C3D += resultadoExpresion[0]
                C3D += "    t" + str(c3dObj.getContadorT()) + " = t" + str(resultadoExpresion[1]) + ";\n"
                temporalT1 = c3dObj.getContadorT()
                c3dObj.addContadorT()
                C3D += "    stack[int(t" + str(temporalTPos) + ")] = t" + str(temporalT1) + ";\n"
                myNewVariable = VariableC3D(self.identificador, "P + " + str(c3dObj.getNumVariables() - 1), TipoVar.APUNTADOR, self.expresion.type, TipoVariable.VARIABLE, self.line, self.column)
            ...
            if isinstance(self.expresion, Primitivo):
                ...
                if isinstance(resultadoExpresion, list):
                    ...
                    temporalT0 = c3dObj.getContadorT()
                    c3dObj.addContadorT()
                    myNewVariable = VariableC3D(self.identificador, myVarOld.getPosition(), TipoVar.VALOR, self.expresion.type, TipoVariable.VARIABLE, self.line, self.column)
                else:
                    C3D += resultadoExpresion
                    C3D += c3dObj.endString()
                    temporalT0 = c3dObj.getLastContadorT()
                    myNewVariable = VariableC3D(self.identificador, myVarOld.getPosition(), TipoVar.APUNTADOR, self.expresion.type, TipoVariable.VARIABLE, self.line, self.column)
                C3D += "    stack[int(t" + str(temporalTPos) + ")] = t" + str(temporalT0)  + ";\n"
            elif isinstance(self.expresion, Aritmetica):
                contadorTP = c3dObj.getContadorT()
                resultadoExpresion = self.expresion.getC3D(c3dObj)
